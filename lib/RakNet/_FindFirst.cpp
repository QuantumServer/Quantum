...
long _findfirst(const char *name, _finddata_t *f) {
    CppRakLib::RakString nameCopy = name;
    CppRakLib::RakString filter;
    ...
    _findinfo_t* fi = CppRakLib::OP_NEW<_findinfo_t>( _FILE_AND_LINE_ );
    fi->filter    = filter;
    fi->dirName   = nameCopy;
    fi->openedDir = dir;
...
int _findnext(long h, _finddata_t *f) {
    ...
    while (true) {
        ...
        CppRakLib::RakString fullPath = fi->dirName + entry->d_name;             
        if (stat(fullPath, &filestat) != 0) {
            CPPRAKLIB_DEBUG_PRINTF("Cannot stat %s\n", fullPath.C_String());
            continue;
        }
...
int _findclose(long h) {
    ...
    CppRakLib::OP_DELETE(fi, _FILE_AND_LINE_);
    return 0;   
}
